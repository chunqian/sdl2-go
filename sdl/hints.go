...
func SDL_SetHintWithPriority(name, value string, hp SDL_HintPriority) bool {
	...
	cValue := createCString(SDL_GetMemoryPool(), value)
	defer destroyCString(SDL_GetMemoryPool(), cName)
	defer destroyCString(SDL_GetMemoryPool(), cValue)

	cRet := C.SDL_SetHintWithPriority(cName, cValue, cHintPriority(hp))
	return int(cRet) > 0
}
...
func SDL_SetHint(name, value string) bool {
	...
	cRet := C.SDL_SetHint(cName, cValue)
	return int(cRet) > 0
}
...
func SDL_GetHint(name string) string {
	...
	return createGoString(C.SDL_GetHint(cName))
}
...
func SDL_DelHintCallback(name string) {
	...
	C.delHintCallback(cName)
}

//export goHintCallback
func goHintCallback(cName, cOldValue, cNewValue *cChar) {
	name := createGoString(cName)
	oldValue := createGoString(cOldValue)
	newValue := createGoString(cNewValue)
